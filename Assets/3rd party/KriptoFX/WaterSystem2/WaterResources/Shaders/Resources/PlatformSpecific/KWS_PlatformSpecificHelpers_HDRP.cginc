...
#ifndef KWS_SHARED_API_IGNORE_INCLUDES
...
float3x3 KWS_WorldToInfluenceSpace(EnvLightData lightData)
{
	...
	inline float4 KWS_ComputeNonStereoScreenPos(float4 pos)
	{
		float4 o = pos * 0.5f;
	...
	{
		float4 o = KWS_ComputeNonStereoScreenPos(pos);
	...
	{
		UNITY_BRANCH
		if (KWS_OverrideSkyColor == 1) return KWS_CustomSkyColor.xyz * _ReflectionLightingMultiplier;
		else return SampleSkyTexture(reflDir, lod, 0).xyz * exposure * _ReflectionLightingMultiplier;
	}
...
inline float EyeDepthToRawUniversal(float eye)
{
	...
	inline void GetInternalFogVariables(float4 pos, float3 viewDir, float surfaceDepthZEye, float3 worldPos, out half3 fogColor, out half3 fogOpacity)
	{
	#ifndef KWS_SHARED_API_IGNORE_INCLUDES
