...
#include "Packages/com.occasoftware.buto/Shaders/Resources/Buto.hlsl"
   ...
   #include "Assets/WeatherMaker/Prefab/Shaders/WeatherMakerFogExternalShaderHlslInclude.hlsl"
#endif
...
{
    return KWS_WorldSpaceCameraPos.xyz;
    //return _WorldSpaceCameraPos.xyz; //cause shader error in 'Hidden / KriptoFX / KWS / VolumetricLighting' : Program 'frag', error X8000 : D3D11 Internal Compiler Error : Invalid Bytecode :
    //source register relative index temp register component 1 in r7 uninitialized. Opcode #61 (count is 1 - based) at line 15 (on vulkan)
...
{
    return length(KWS_WorldSpaceCameraPos.xyz - worldPos.xyz);
}

inline float GetWorldToCameraDistanceXZ(float3 worldPos)
{
    return length(KWS_WorldSpaceCameraPos.xz - worldPos.xz);
}
...
{
    return normalize(KWS_WorldSpaceCameraPos.xyz - worldPos.xyz);
}
...
inline float4 WorldPosToScreenPos(float3 pos)
{
    ...
    float4 projected = mul(KWS_MATRIX_VP, float4(pos, 1.0f)); //UNITY_MATRIX_VP Todo check 
...
inline float EyeDepthToRawUniversal(float eye)
{
    eye = max(eye, 0.001);
    float persp = (1.0 / eye - _ZBufferParams.w) / _ZBufferParams.z;
    float ortho = 1.0 - (eye - _ProjectionParams.y) / (_ProjectionParams.z - _ProjectionParams.y);

    return saturate(lerp(persp, ortho, unity_OrthoParams.w));
}
...
// inline float Linear01DepthUniversal(float z)
// {
//     float persp = 1.0 / (_ZBufferParams.x * z + _ZBufferParams.y);
//
//     float ortho = ( (_ProjectionParams.z - _ProjectionParams.y) * (1 - z) )
//                   / (_ProjectionParams.z - _ProjectionParams.y);
//
//     return lerp(persp, ortho, unity_OrthoParams.w);
// }

inline void GetInternalFogVariables(float surfaceDepthZEye, out half3 fogColor, out half3 fogOpacity)
{
...
inline float3 ComputeInternalFog(float3 sourceColor, float3 fogColor, float3 fogOpacity)
{
...
inline half3 ComputeThirdPartyFog(half3 sourceColor, float3 worldPos, float2 screenUV, float screenPosZ)
{
        ...
        sourceColor.rgb = ApplyClouds(sourceColor.rgb, screenUV.xy, worldPos.xyz);
        ...
        sourceColor = BlendStylizedFog(worldPos, half4(sourceColor.xyz, 1)).xyz;
    #elif defined(BUTO)
        sourceColor = ButoFogBlend(screenUV, LinearEyeDepthUniversal(screenPosZ), sourceColor);
