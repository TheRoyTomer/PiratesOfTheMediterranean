...
void Tex2DStochastic_float(UnityTexture2D tex, float2 uv, out float4 result)
{
		...
		float4 tex1 = SAMPLE_TEXTURE2D_GRAD(tex, tex.samplerstate, uv + KWS_hash2D2D(hash1.xy), dx, dy) * weight.x;
		float4 tex2 = SAMPLE_TEXTURE2D_GRAD(tex, tex.samplerstate, uv + KWS_hash2D2D(hash2.xy), dx, dy) * weight.y;
		float4 tex3 = SAMPLE_TEXTURE2D_GRAD(tex, tex.samplerstate, uv + KWS_hash2D2D(hash3.xy), dx, dy) * weight.z;
