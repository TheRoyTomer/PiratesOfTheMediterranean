Shader "Hidden/KriptoFX/KWS/VolumetricLighting"
{
	...
	#define CAUSTIC_LOD_OCEAN 3.5
	#define CAUSTIC_LOD_DYNAMIC_WAVES_ZONE 3.0
	...
	half RaymarchCaustic(RaymarchData raymarchData, float3 currentPos, float3 lightForward, bool isDynamicWavesZone)
	{
		...
		float2 uv = (currentPos.xz - offsetLength * lightForward.xz) * KWS_WavesDomainScaledSizesInv[causticSlice];

		half caustic  = 0;
		if (isDynamicWavesZone) caustic = GetCausticDynamicWavesZoneLod1(uv, CAUSTIC_LOD_DYNAMIC_WAVES_ZONE) * 5 - KWS_CAUSTIC_MULTIPLIER;
		else caustic = GetCausticOceanLod(uv, causticSlice, CAUSTIC_LOD_OCEAN) - KWS_CAUSTIC_MULTIPLIER;
	...
	inline void UpdateLocalZones(inout RaymarchData raymarchData)
	{
			...
			float noise = KWS_InterleavedGradientNoise(raymarchData.uv * _ScreenParams.xy, KWS_Time) * 2 - 1;
	...
	inline void UpdateColoredZones(inout RaymarchData raymarchData)
	{
		#ifdef KWS_USE_COLORED_DYNAMIC_WAVES
			float distanceToCamera = GetWorldToCameraDistance(raymarchData.currentPos);
			float3 dynamicWavesMapUV = GetDynamicWavesMapUV(raymarchData.currentPos, distanceToCamera);
			...
			if (isDynamicWavesZone)
			{
				...
				zoneColorData.a = saturate(zoneColorData.a);
				...
				raymarchData.turbidityColor = lerp(raymarchData.turbidityColor, zoneColorData.rgb, zoneColorData.a);
	...
	RaymarchData InitRaymarchData(vertexOutput i)
	{
		...
		float waterMask = GetWaterMask(i.uv);
			...
			float2 clipMaskThickness = GetWaterMaskThickness(i.uv);
            bool insideAir = (clipMaskThickness.x == 0.0 && clipMaskThickness.y > 0);
			float clipMask = GetWaterClipMask(i.uv);
            if (!GetUnderwaterMask(waterMask) && GetUnderwaterMask(clipMask) && !insideAir) waterMask = clipMask;
			
		#endif
		
		if (waterMask == 0) return data;

		float sceneZ = GetSceneDepth(i.uv);
		float waterDepth = GetWaterDepth(i.uv);
		
		if (GetUnderwaterMask(waterMask) < 0.5 && sceneZ > waterDepth) return data;
		
		#ifdef KWS_USE_CLIP_MASKING
			data.waterVolumeDepth = GetWaterVolumeDepth(i.uv, sceneZ, waterMask, waterDepth, clipMaskThickness, clipMask);
		#else
			data.waterVolumeDepth = GetWaterVolumeDepth(i.uv, sceneZ, waterMask, waterDepth);
		#endif
		//if (data.waterVolumeDepth.x == 0) return data;
		...
		data.waterHeight = KWS_WaterLevel;
		...
		data.offset = KWS_InterleavedGradientNoise(i.vertex.xy, KWS_Time);

		data.currentPos = data.rayStart + data.step * data.offset;
		data.uv = i.uv;

		UpdateLocalZones(data);
		UpdateColoredZones(data);
	...
	inline void IntegrateAdditionalLight(RaymarchData raymarchData, inout float3 scattering, inout float transmittance, float atten, float3 lightPos, float3 step, inout float3 currentPos)
	{
		float3 posToLight = normalize(GetCameraRelativePosition(currentPos) - lightPos.xyz);
			...
			{
				atten += atten * RaymarchCaustic(raymarchData, currentPos, posToLight, false);
			}
		#endif
	...
	#pragma multi_compile_fragment _ KWS_USE_VOLUMETRIC_DIR_CAUSTIC KWS_USE_VOLUMETRIC_FULL_CAUSTIC
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag(vertexOutput i) : SV_Target0
			{
				...
				RaymarchData raymarchData = InitRaymarchData(i);
				RaymarchResult raymarchResult = (RaymarchResult)0;

				if(raymarchData.transparent == 0) discard;
		...
		Pass
		{
			...
			void frag(vertexOutput i, out half3 additionalData : SV_Target0, out half3 surfaceLight : SV_Target1)
			{
				...
				float sceneZ = GetSceneDepth(i.uv);
				float waterDepth = GetWaterDepth(i.uv);
				float2 waterVolumeDepth = GetWaterVolumeDepth(i.uv, sceneZ, waterMask, waterDepth);

				if(waterVolumeDepth.x == 0.0)
				{
					surfaceLight = GetAmbientColor(GetExposure());
					additionalData = 0.0;
					return;
					//discard;
				}
				...
				float3 surfaceStartPos = GetUnderwaterMask(waterMask) ? endPos : startPos;

				float4 fullSurfaceLight = KWS_ComputeLighting(surfaceStartPos, 0, false, i.uv);
