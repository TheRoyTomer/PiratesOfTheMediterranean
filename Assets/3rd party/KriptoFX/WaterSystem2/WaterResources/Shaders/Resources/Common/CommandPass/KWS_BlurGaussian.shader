Shader "Hidden/KriptoFX/KWS/BlurGaussian"
{
	...
	float _Threshold;
	float _DisableBlackColorPropagation;
	...
	half4 frag_separable_low(float2 uv, float2 direction) : SV_Target
	{
		...
		for (int idx = -2; idx <= 2; idx += 1)
		{
			...
			half4 sample = SAMPLE_TEXTURE_LOD(_SourceRT, sampler_linear_clamp, uv + offset * idx, 0);
			
			float valid = lerp(1.0, sample.r > 0.001 ? 1.0 : 0.0, _DisableBlackColorPropagation);
			float w = weight * valid;

	        s += sample * w;
	        totalWeight += w;
		}
		return totalWeight > 1e-5 ? s / totalWeight : 0;
	}
