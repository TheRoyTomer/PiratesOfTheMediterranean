Shader "Hidden/KriptoFX/KWS/KWS_DynamicWavesSplashParticles"
{
    ...
    static const float2 quadOffsets2[6] =
    {
        float2(-1, -1),
		float2(-1, 1),
		float2(1, 1),
    ...
    void GetDynamicWavesFoamParticlesVertexPosition(SplashParticle particle, uint vertexID, float farDistance, out float3 vertex, out float2 uv, out float particleSpeed, out float normalizedLifeTime, out float particleSize)
    {
        ...
        float verticalVelocityStretch = saturate(KWS_Pow5(gravityFactor) * particleSpeed * 0.1) * 20;
        ...
        particleSize = lerp(minSize * KWS_SplashParticlesScale, SPLASH_SIZE_MAX * saturate(KWS_SplashParticlesScale + 0.35), KWS_Pow5(particle.initialRandom01) * saturate(particle.initialSpeed * 0.2));
        ...
        particleHeightOffset += lerp(particleSize * 0.8, particleSize * 0.4, particle.shorelineMask);
...
//		}
        ...
        Pass
        {
            ...
            struct v2f
            {
                ...
                float4 overrideColor : TEXCOORD5;
                #endif
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                float2 screenUV = i.screenPos.xy / i.screenPos.w;
					...
					if (ShouldClipWaterSurface(screenUV, i.screenPos.z / i.screenPos.w)) discard;
				#endif
                ...
                float softParticlesFade = saturate(1 * (min(waterDepth, sceneDepth) - surfaceDepth));
                ...
                //return float4(lerp(pow(splashDepth, Test4.x),  1, softParticlesFade * softParticlesFade), 0, 0, 1);
					...
					i.color.rgb = KWS_ComputeLighting(i.worldPos, 0.1, true, screenUV).xyz;
                    
                #endif

                #ifdef KWS_LOCAL_DYNAMIC_WAVES_USE_COLOR
                    waterColor = lerp(waterColor, i.overrideColor.rgb, saturate(i.overrideColor.a * 2));
                    splashShine =  lerp(splashShine, i.overrideColor.rgb * 0.1, i.overrideColor.a * 0.75);
                    splashAlpha = lerp(splashAlpha, saturate(splashAlpha * 3),  i.overrideColor.a);
        ...
        Pass
        {
            Tags
            {
                "LightMode" = "ShadowCaster" "Queue" = "AlphaTest+1"
            }
            ...
            #pragma multi_compile  _ KWS_USE_SPLASH_SHADOW_CAST_FAST
            #pragma multi_compile_fragment _ KWS_USE_CLIP_MASKING
